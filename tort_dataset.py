...
def generate_dataset(db_size:int, lbl_distr:float=0.5) -> pd.DataFrame:
    '''
    Generates a dataset with a predefined size and distribution.
    :param db_size: The number of instances in the dataset
    :param lbl_distr: Ratio of how many instances have a false label
...
def preprocess(dataset:pd.DataFrame, return_df=False) -> Tuple[np.ndarray, np.ndarray]:
    '''
    ...
    '''
    X = dataset.loc[:, dataset.columns != 'dut']
    if not return_df:
        X = X.values
...
def generate_cause_dataset() -> pd.DataFrame:
    '''
    Generates a dataset in which all conditions are satisfied except cause.
    :return: Pandas DataFrame, the new dataset 
    '''
...
def generate_damages_dataset() -> pd.DataFrame:
    '''
    Generates a dataset in which all conditions are satisfied except damages.
    :return: Pandas DataFrame, the new dataset 
    '''
    ...
    # Remove instances without cause (cau)
    df = df[df.cau == 1]
...
def generate_violationpurpose_dataset() -> pd.DataFrame:
    '''
    Generates a dataset in which all conditions are satisfied except the violation/purpose.
    :return: Pandas DataFrame, the new dataset 
    '''
