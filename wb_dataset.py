...
def create_dataset(db_size:int, instance_function=create_instance, rule_number:int=None,
                   lbl_distr:int=0.5, n_noise:int=0, num_rules:int=6) -> pd.DataFrame:
    '''
    ...
    :param lbl_distr: the label distribution eligible/ineligible between 0 and 1
    ...
    # The satisfied cases
    for n in range(0, math.floor(db_size * (1 - lbl_distr))):
        db.append(instance_function(None))
    
    # The unsatisfied cases
    for n in range(0, math.ceil(db_size * lbl_distr)):
...
def create_contributions_dataset(db_size=1, n_noise=0, instance_function=create_instance_single_fail):
    '''
    Returns a dataset with instances in which all conditions are satisfied
    except for condition 2, which is generated across the full range.
    :param db_size: The number of times the full range is generated
    :param n_noise: The amount of noise variables that are added to the data
    :return: The dataset as a pandas DataFrames    
    '''
    db = []
    contributions_combinations = list(it.product(['yes', 'no'], repeat=5))
    for idx in range(0, db_size):
        for contributions in contributions_combinations:
            eligible = contributions.count('yes') >= THRESOLD_PAID_CONTRIBUTIONS
            instance = instance_function(None)
            instance.update({'paid_contribution_1': contributions[0],
                             'paid_contribution_2': contributions[1],
                             'paid_contribution_3': contributions[2],
                             'paid_contribution_4': contributions[3],
                             'paid_contribution_5': contributions[4],
                             'eligible': eligible,
                            })
            db.append(instance)
    db = add_noise(db, n_noise)
...
def create_spouse_dataset(db_size=1, n_noise=0, instance_function=create_instance_single_fail):
    '''
    Returns a dataset with instances in which all conditions are satisfied
    except for condition 3, which is generated across the full range.
    :param db_size: The number of times the full range is generated
    :param n_noise: The amount of noise variables that are added to the data
    :return: The dataset as a pandas DataFrames    
    '''
    ...
    db = []
    for idx in range(0, db_size):
        for spouse in [True, False]:
            instance = instance_function(None)
            instance.update({'is_spouse': spouse,
                             'eligible': spouse,
                            })
            db.append(instance)
    db = add_noise(db, n_noise)
...
def create_absent_dataset(db_size=1, n_noise=0, instance_function=create_instance_single_fail):
    '''
    Returns a dataset with instances in which all conditions are satisfied
    except for condition 4, which is generated across the full range.
    :param db_size: The number of times the full range is generated
    :param n_noise: The amount of noise variables that are added to the data
    :return: The dataset as a pandas DataFrames    
    '''
    ...
    for idx in range(0, db_size):
        for absent in [True, False]:
            instance = instance_function(None)
            instance.update({'is_absent': absent,
                             'eligible': not absent,
                            })
            db.append(instance)
    db = add_noise(db, n_noise)
...
def create_resources_dataset(db_size=1, n_noise=0, instance_function=create_instance_single_fail):
    '''
    Returns a dataset with instances in which all conditions are satisfied
    except for condition 5, which is generated across the full range.
    :param db_size: The number of times the full range is generated
    :param n_noise: The amount of noise variables that are added to the data
    :return: The dataset as a pandas DataFrames    
    '''
    ...
    db = []
    resources_combinations = [resources for resources in range(MIN_CAPITAL_RESOURCES, MAX_CAPITAL_RESOURCES+50, 50)]
    for idx in range(0, db_size):
        for resources in resources_combinations:
            eligible = resources <= THRESHOLD_CAPITAL_RESOURCES
            instance = instance_function(None)
            instance.update({'capital_resources': resources,
                             'eligible': eligible,
                            })
            db.append(instance)
    db = add_noise(db, n_noise)
...
def is_eligible(instance:dict) -> bool:
    ...
    :return: True if the instance is eligible, false otherwise
    '''
